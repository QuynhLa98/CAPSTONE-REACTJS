...
const LichChieuPhim = () => {
    ...
    // create state save data from redux new select
    const [cumRap, setCumRap] = useState(lichChieuPhim?.heThongRapChieu);
    // useState not to data redux , select data here
    // => create useEffect, setState = data send request
                                ...
                                <RapChieu>
                                    ...
                                    <div>
                                        {cumRap.lichChieuPhim.map((phim, index) => (
                                            <Link  key={index} to={`/chitietphongve/${phim.maLichChieu}`}>{phim.ngayChieuGioChieu.slice(11, 16)}</Link>
                                        ))}
                                    </div>
                                </RapChieu>
                            </React.Fragment>
