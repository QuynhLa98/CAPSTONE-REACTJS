                    ...
                    {(dsPhim || heThongRap[0]?.lstCumRap[0])?.danhSachPhim.map((phim, index) => {
                        if (phim.dangChieu) {
                            return (
                                <div key={index}>
                                    <Poster>
                                        <Link to={`/chitietphim/${phim.maPhim}`}>
                                            <img src={phim.hinhAnh} alt="" />
                                        </Link>
                                    </Poster>
                                    <Details>
                                        <h4 onClick={() => { navigate(`/chitietphim/${phim.maPhim}`) }}>{phim.tenPhim}</h4>
                                        <p>2D phụ đề</p>
                                        <div>
                                            {phim.lstLichChieuTheoPhim.map((item, index) => (
                                                <Link to={`/chitietphongve/${item.maLichChieu}`} key={index}>{item.ngayChieuGioChieu.slice(11, 16)}</Link>
                                            ))}
                                        </div>
                                    </Details>
                                </div>
